...
- **Read boundary:** the bootstrap assigns authentik's official **`authentik Read-only`** role, which grants only per-model `view_<model>` permissions — so secret-reveal permissions (`view_token_key`, `view_certificatekeypair_key`, …) are excluded *by construction*, preventing exfiltration of API tokens and certificate private keys. See `docs/agent-security-model.md` § 5–7 for threat model and design rationale.
